...
# 3964r Protokoll Treiber fur RS232
...
#---------------------------------------------------------------------------
# Schrittkette fur 3964r Protokoll
# Grundklasse ist Stepchain, elementare Schrittkette
#
# Die Prozedur 3964R ist ein asynchrones, bitserielles ubertragungsverfahren. uber die Verbindung werden
# Steuer- und Nutzinformationszeichen gesendet. Um jedes Zeichen beim Empfanger wiederzuerkennen,
# und um die fehlerfreie ubertragung zu kontrollieren, werden den gesendeten Zeichen weitere Bits voranbzw.
...
# SO = Stoppbit
#
# Die Steuerzeichen fur die Prozedur 3964R sind der Norm DIN 66003 fur den 7-Bit-Code entnommen. Sie
# werden allerdings mit der Zeichenlange 8 Bit ubertragen (Bit I7 = 0). Am Ende jedes Datenblocks wird zur
# Datensicherung ein Prufzeichen(BCC) gesendet.
# Das Blockprufzeichen wird durch eine exklusiv-oder-Verknupfung uber alle Datenbytes der
# Nutzinformation, inclusive der Endekennung DLE, ETX gebildet.
# Fur die Informationszeichen ist kein Code vorgeschrieben (Codetransparenz).
#
# *****************************
# Senden mit der Prozedur 3964R
# Zum Aufbau der Verbindung sendet die Prozedur 3964R das Steuerzeichen STX aus. Antwortet das
# Peripheriegerat vor Ablauf der Quittungsverzugzeit (QVZ) von 2 sec mit dem Zeichen DLE, so geht die
# Prozedur in den Sendebetrieb uber. Antwortet das Peripheriegerat mit NAK, einem beliebigen anderen
...
# Nutzinformationszeichen mit der gewahlten ubertragungsgeschwindigkeit an das Peripheriegerat
# gesendet. Das Peripheriegerat soll die ankommenden Zeichen in Ihrem zeitlichen Abstand uberwachen.
...
# DLE zweimal in die Prufsumme ubernommen.
#
# Nach erfolgtem senden des Pufferinhalts fugt die Prozedur die Zeichen DLE, ETX und BCC als
# Endekennung an und wartet auf ein Quittungszeichen. Sendet das Peripheriegerat innerhalb der
# Quittungsverzugszeit QVZ das Zeichen DLE, so wurde der Datenblock fehlerfrei ubernommen. Antwortet
# das Peripheriegerat mit NAK, einem beliebigen anderen Zeichen (ausser DLE), einem gestorten Zeichen
...
#
# Sendet das Peripheriegerat wahrend einer laufenden Sendung das Zeichen NAK, so beendet die
# Prozedur den Block und wiederholt in der oben beschriebenen Weise.
#
# Beispiel fur einen fehlerlosen Datenverkehr:
# Prozedur 3964R Peripheriegerat
# STX           ->
#               <- DLE
# 1. Zeichen    ->
#               ->
...
# die Prozedur auf den Verbindungsaufbau durch das Peripheriegerat. Empfangt die Prozedur ein STX und
# steht ihr ein leerer Eingabepuffer zur Verfugung, wird mit DLE geantwortet.
#
# Nachfolgende Empfangszeichen werden nun in dem Eingabepuffer abgelegt. Werden zwei aufeinander
# folgende Zeichen DLE empfangen, wird nur ein DLE in den Eingabepuffer ubernommen.
# Nach jedem Empfangszeichen wird wahrend der Zeichenverzugszeit (ZVZ) auf das nachste Zeichen
# gewartet. Verstreicht die Zeichenverzugszeit ohne Empfang, wird das Zeichen NAK an das
# Peripheriegerat gesendet und der Fehler an den Interpreter gemeldet.
#
# Mit erkennen der Zeichenfolge DLE, ETX und BCC beendet die Prozedur den Empfang und sendet DLE
# fur einen fehlerfrei (oder NAK fur einen fehlerhaft) empfangenen Block an das Peripheriegerat.
# Treten wahrend des Empfangs ubertragungsfehler auf (verlorenes Zeichen, Rahmenfehler), wird der
# Empfang bis zum Verbindungsabbau weitergefuhrt und NAK an das Peripheriegerat gesendet. Dann wird
# eine Wiederholung des Blocks erwartet. Kann der Block auch nach insgesamt sechs Versuchen nicht
# fehlerfrei empfangen werden, oder wird die Wiederholung vom Peripheriegerat nicht innerhalb der
...
# Antwortet ein Gerat auf den Sendewunsch (Zeichen STX) seines Peripheriegerates innerhalb der
# Quittungsverzugszeit QVZ nicht mit der Quittung DLE oder NAK, sondern ebenfalls mit dem Zeichen STX,
# liegt ein Initialisierungskonflikt vor. Beide Gerate mochten einen vorliegenden Sendeauftrag ausfuhren.
# Das Gerat mit der niederen Prioritat stellt seinen Sendeauftrag zuruck und antwortet mit dem Zeichen
# DLE. Das Gerat mit der hoheren Prioritat sendet daraufhin seine Daten in der vorher beschriebenen
# Weise. Nach dem Verbindungsabbau kann das Gerat mit der niederen Prioritat seinen Sendeauftrag
# ausfuhren.
#
# niedrige Prioritat     hohere Prioritat
#   STX           ->
#                 <-     STX  (Konflikt)
#   DLE           -> 
#                 <-    1. Zeichen
...
#
#
# Klassendifinition fur den 3964r Treiber
...
class Dust3964r (stepchain,Serial):
    ...
    MODE        = True    # Mit Blockprufzeichen
    CFG_PRIO    = True    # Treiber lauft mit hoher Prioritat
    CFG_PRINT   = True    # Modus Print eingeschalet (
    ETX_EN      = False   # Sequenzer erkennt, ob ein ETX nach einem DLE gultig ist
    BCC_EN      = False   # Sequenzer erkennt, das nach DLE, ETX nun das BCC folgen muss
    RealRun     = True    # Lauf im Simulator = false, in Realitat True
    RUN         = False   # Der treiber lauft
    STX         = b"\x02" # chr(0x02)
    ETX         = b"\x03" # chr(0x03)
    DLE         = b"\x10" # chr(0x10)
    NAK         = b"\x15" # chr(0x15)
    LOPRIO      = False   # niedrige Prio
    HIPRIO      = True    # hohe Prioritat
    M3964       = False   # Treiber lauft als 3964 ohne BCC Blocksumme
    M3964R      = True    # Treiber lauft als 3964r mit BCC Blocksumme

    def __init__ (self,port="/dev/ttyUSB0",baudrate=2400,QVZ=2.0,ZVZ=0.22,BWZ=4.0,CWZ = 3.0,SPZ=0.5,SLP= 1.4,MAXSEND=6,MAXCONNECT=6,PRIO=HIPRIO, MODE=M3964R):
        ...
        self.SPZ       = SPZ        # SendePause zeit (nach einem erfolgreichem Senden warten bis nachstes Senden
        ...
        self.MODE      = MODE       # Treibermodus einstellen (Serienmassig nach dem Start: 3964r mit Blocksumme
    ...
    # hiermit kann der Modus des Treibers umgeschaltet werden. der Aufruf kann nur nach dem INIT gemacht werden, nicht wahrend des Laufens
    # Mogliche Mode sind: PRIO = LO   : Bei einem INIT Konflikt stellt Treiber seinen Sendewunsch zuruck
    #                     PRIO = HI   : Bei einem Init Konflikt besteht der Treiber auf Bestatigung Sendebereitschaft durch die Gegenseite
    #                     MODUS= 3964 : ubertragung ohne Blockprufkennung BCC
    #                     MODUS= 3964r: ubertragung mit Blockprufkennung BCC
    def mode (self,PRIO,MODE):
        if not self.RUN:
            self.CFG_PRIO= PRIO
            self.MODE    = MODE

    # Berechnet das XOR CRC fur den ubergebenen buff
    # buffer sollte ein bytestring sein
    # der Ruckgabewert ist ein bytestring
    def crc (self,buffer):
        bcc= 0x00
        for c in buffer:
            bcc ^= c
    ...
    # Ruckgabewert ist ebenfalls ein bytestring
    def outframe (self,buffer):
        # Ein DLE in Datenstring fuhrt zur Verdopplung von DLE
        # DLE und ETX sind der Frame vom 3964r Protokoll
        puffer= buffer.replace (self.DLE,self.DLE+self.DLE)+ self.DLE + self.ETX
        # prufen, welches Protokoll: 3964= ohne BCC, 3964r mit BCC
        if self.MODE:
            puffer+=self.crc (puffer)
        return puffer

    # Befreit das Telegramm von dem 3964r Frame, pruft die Checksumme
    # Der buffer muss ein bytestring sein
    # Ruckgabe: NONE, wenn irgendein Fehler aufgetreten ist, Telegramm verstummelt, Checksum falsch
    # Wenn alles OK, Ruckgabe des Telegramms als Bytestring
    def inframe (self,buffer):
        ...
        if self.MODE:
            ...
            if bufferBCC!=self.crc (stream):
                return None
        try:
            # Nun prufen, ob am Ende vom Stream DLW und ETX drinsind
    ...
    def sendstream (self,sendepuffer):       
        ...
        if self.CFG_PRINT:
            ...
            for c in buffer:
                print("%3.2X"% c + "s",end="")
        # der puffer wurde uber die rs232 ausgegeben

    # Fehler in der Kommunikation: NAK ausgeben
    # Bei einem NAK wird immer auch ein flush ausgefuhrt
    def errNAK (self):
    ...
    # eine Verzogerungszeit fur das nachste Senden wird definiert
    ...
    # Virtuelle Routine, muss uberladen werden vom child    
    ...
    # Routine Pruft, ob im Sendepuffer ein Auftrag vorhanden ist
    def isJob (self):
        return self.telegrammOut!=[]

    # Routine fugt einen neunen Sendeauftrag in den Puffer ein
    def newJob (self,job):
    ...
    # Routine nimmt den altesten Sendeauftrag aus der Liste und gibt diesen Zuruck
    # existiert kein Job, wird NONE zuruckgegeben
    def getJob (self):    
        if not self.isJob ():
    ...
    # Steht kein aktuelles Kommando zur Ausfuhrung an und ist inWaiting() <>0 (Zeichen im Buffer)
    # Dann neuer Schritt = 1 (Empfang uberprufen)
    def schritt_0 (self):
        ...
        if self.RS232.inWaiting () and self.RealRun:
            ...
            # An dieser Stelle kann und darf es hochstens das Zeichen STX sein
            ...
            if char != self.STX:
                ...
                if not self.CFG_PRIO or not self.SEND_EN: # Treiber hat niedrige PRIO oder nix zum senden
                    ...
                    t.sleep (self.SLP) # Fur die erlaubte Antwortzeit legt sich der Prozess schlafen    
                    self.setnewstep (4) # Verbindungsaufbau 3964r lauft nun ready to receive
                elif self.SEND_EN:
                    if self.CFG_PRINT:
    ...
    # Alles was nicht DLE ist grund fur ein NAK (Hi prio)
    def schritt_1 (self):
        if self.stepdauer>self.QVZ:
            # Quittungsverzugszeit ist abgelaufen
            if self.CFG_PRINT:
                print (" 15s [NAK: QVZ-START]")
            self.connectERR +=1 # Verbindungsaufbaufehler um 1 erhohen
            ...
            if c!= self.DLE:
                ...
                self.SetSendDelay (self.CWZ)
                self.errNAK ()
            else: # es war ein DLE, senden ausfuhren
                self.sendstream (self.sendbuff)
                self.setnewstep (2)

    # Schritt 2: Gesendeter Datenstream muss mit DLE vom Empfanger bestatigt werden
    ...
    def schritt_2 (self):
        if self.stepdauer>self.QVZ:
            ...
            self.sendERR +=1 # sendefehler um 1 erhohen
            ...
            if c!= self.DLE:            
                ...
                self.sendERR +=1 # Verbindungsaufbaufehler um 1 erhohen
                self.SetSendDelay (self.BWZ)
                self.errNAK ()
                ...
                self.sendbuff=b"" # Sendepuffer loschen, das telegramm austragen
    ...
    def schritt_3 (self):
        if self.stepdauer>self.QVZ:
            ...
            if c== self.DLE:
                ...
                # wunderbar, alles, ok, wir konnen senden
                self.sendstream (self.sendbuff)
                # Nach dem Senden muss mit DLE vom empfanger bestatigt werden
                self.setnewstep (2)
            elif c== self.STX:
                # Wir bekommen als Antwort auf unser STX ebenfalls ein STX zuruck
                # der Klassische Initialisierungskonflikt
                if not self.CFG_PRIO:
                    # Unser Treiber hat Low Prio also alles OK, wir mussen mit DLE antworten
                    # Es folgt nun ganz normales Empfangen
                    if self.CFG_PRINT:
                        print (" 02r",end="")
                    self.setnewstep (4)
                else: # Nu gibts ein Problem.
                    # Unserer Treiber lauft auf High Prio, und die Gegenseite setzte auch ein STX ab
                    ...
                    self.connectERR +=1 # connectfehler um 1 erhohen
                    self.SetSendDelay (0)
                    self.errNAK ()
            else:
                print ("%3.2X"% ord (c) + "r 15s [NAK: DLE-START]")
            ...
            while self.RS232.inWaiting():
                ...
                if self.BCC_EN:
                    ...
                    if rec is None:
                        ...
                        self.RS232.flushInput ()
                        self.RS232.flushOutput ()                      
                        t.sleep (self.SLP) # Fur die erlaubte Quittungsverzugszeit legt sich der Prozess mal schlafen
                        self.RS232.write (self.DLE)
                    self.setnewstep (0)    
                    break
                elif (c==self.DLE):
                    ...
                    self.BCC_EN= True # Endekennung gultig erkannt, nun das BCC als letztes Zeichen
                else:
                    self.STX_EN=False
                    self.BCC_EN=False
        return
    ...
    # Wird immer ausgefuhrt vor den Schritten
    def schritt (self):
